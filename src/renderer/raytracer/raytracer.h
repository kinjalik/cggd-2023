...
namespace cg::renderer
{
			...
			const VB& vertex_a, const VB& vertex_b, const VB& vertex_c)
	{
		...
		na =  float3{vertex_a.nx, vertex_a.ny,
					vertex_a.nz};

		nb =   float3{vertex_b.nx, vertex_b.ny,
					vertex_b.nz};

		nc =   float3{vertex_c.nx, vertex_c.ny,
					vertex_c.nz};

		ambient =   float3{vertex_a.ambient_r, vertex_a.ambient_g,
						 vertex_a.ambient_b};

		diffuse =   float3{vertex_a.diffuse_r, vertex_a.diffuse_g,
						 vertex_a.diffuse_b};

		emissive =  float3{vertex_a.emissive_r, vertex_a.emissive_g,
						  vertex_a.emissive_b};
	}
												...
												size_t in_height)
	{
		width = in_width;
		height = in_height;
		history = std::make_shared<
		          cg::resource<float3>>(width, height);
	}
...
#pragma omp parallel for
			for (int x = 0; x < width; ++x) {
				for (int y = 0; y < height; ++y) {
					...
					history->item(x, y) += sqrt(payload.color.to_float3() /
												accumulation_num);
	...
	float2 raytracer<VB, RT>::get_jitter(int frame_id)
	{
		...
		fraction = inv_base;
		while (index > 0) {
			result.y += static_cast<float>(index % base_y)
						* fraction;
			index /= base_y;
			fraction *= inv_base;
		}
	...
	inline bool aabb<VB>::aabb_test(const ray& ray) const
	{
		...
		float3 t0 = (aabb_max - ray.position)
					* inv_ray_dir;
		float3 t1 = (aabb_min - ray.position)
					* inv_ray_dir;
